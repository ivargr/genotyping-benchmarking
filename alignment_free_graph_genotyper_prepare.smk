...
rule make_genotype_matrix:
    ...
    shell:
        ...
        "obgraph make_genotype_matrix -g {input.graph} -v {input.vcf} -o {output.genotype_matrix} -n {wildcards.n_individuals} -m $n_variants && "
        "obgraph analyse_genotype_matrix -G {output.genotype_matrix} -o {output.most_similar_variant_lookup} &&"
        "obgraph make_transition_probabilities -G {output.genotype_matrix} -m {output.most_similar_variant_lookup} -o {output.transition_probs} && "
        "obgraph get_genotype_frequencies -g {output.genotype_matrix} -o {output.genotype_frequencies} -t 40"
